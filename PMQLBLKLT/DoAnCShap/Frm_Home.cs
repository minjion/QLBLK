...
namespace DoAnCShap
{
    public partial class Frm_Home : Form
    {
        ...
        {
            bool KiemTra = false;
            for (int i = 0; i < bus.GetLogin1(Frm_Login.TenTaiKhoan).Rows.Count; i++)
            {
                if (bus.GetLogin1(Frm_Login.TenTaiKhoan).Rows[i][col].ToString() == "True")
                    return KiemTra = true;
            }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (PhanQuyen(28)) btnNhaSanXuat.Enabled = true; else btnNhaSanXuat.Visible = false;
            labelHienThiTenDangNhap.Text = Frm_Login.TenTaiKhoan;
        }
        ...
        private void btnLoaiLK_Click(object sender, EventArgs e)
        {
            if (PhanQuyen(20))
            {
                ...
                openChildForm(new Frm_LoaiLinhKien());
            }
            else
            {
                MessageBox.Show("Bạn Không Có Quyền Truy Cập", "Cảnh Báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
