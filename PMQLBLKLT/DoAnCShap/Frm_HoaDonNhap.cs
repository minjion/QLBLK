...
namespace DoAnCShap
{
    public partial class Frm_HoaDonNhap : Form
    {
        ...
        {
            comboBoxNSX.DataSource = bus.GetNSX("");
        ...
        public void XuLyTextBox(Boolean b1)
        {
            ...
            txbThanhTien.Enabled = b1;
            txbTongThanhToan.Enabled = b1;
            txtMaHDN.Enabled = b1;
        ...
        public void ClearTexBox()
        {
            ...
            txbThanhTien.ResetText();
            txbTongThanhToan.ResetText();
        ...
        public void TongTienSP()
        {
            ...
            txbTongThanhToan.Text = sum.ToString();
            txbTongThanhToan.Text = string.Format("{0:#,##0}", decimal.Parse(txbTongThanhToan.Text));
        }
        ...
        private void btnChonNhap_Click(object sender, EventArgs e)
        {
            ...
            txbThanhTien.Text = tt.ToString("0,00");
            txbThanhTien.Text = string.Format("{0:#,##0}", double.Parse(txbThanhTien.Text));
            ...
            else
            {
                ...
                Add_Datagrid(comboBoxTenLK.Text, int.Parse(textBoxSoLuong.Text), decimal.Parse(textBoxDonGia.Text), decimal.Parse(textBoxChietKhau.Text), decimal.Parse(txbThanhTien.Text), SoLuongConLai);
            }
            TongTienSP();
        ...
        public void UpateThanhTienSP()
        {
            ...
            txbThanhTien.Text = ThanhTienSP.ToString();
        }

        private void Frm_HoaDonNhap_Load(object sender, EventArgs e)
        {
            string condition = Frm_Login.SetValueForText1;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            if (flag == 1)
            {
                ...
                {
                    errorMes.BlinkRate = 100;
                    errorMes.SetError(comboBoxNSX, "Chưa chọn nhà sản xuất");
                    return;
                }
                ...
                if (txbTongThanhToan.Text == "")
                {
                    errorMes.BlinkRate = 100;
                    errorMes.SetError(txbTongThanhToan, "? TT");
                    return;
                }
                else
                {
                    try
                    {
                        ...
                        hdn.MaNSX = comboBoxNSX.SelectedValue.ToString();
                        hdn.MaNV = comboBoxMaNV.SelectedValue.ToString();
                        hdn.NgayLapHDNH = dateTimePickerNgayLapHDN.Value;
                        hdn.TongTien = decimal.Parse(txbTongThanhToan.Text);
                        hdn.TrangThai = "1";
                        bus.AddHoaDon(hdn);
                        string[] b = MaLK.Split(';');
                        for (int i = 0; i < dataGridViewCTHDNH.Rows.Count - 0; i++)
            ...
            if (cbochucvu.SelectedValue.ToString() == "CV01")
            {
                if (flag == 2)
                {
                    ...
                    {
                        errorMes.BlinkRate = 100;
                        errorMes.SetError(comboBoxNCC, "Chưa chọn nhà sản xuất");
                        return;
                    }
                    if (comboBoxMaNV.Text == "")
                    {
                ...
                if (addnew == true)
                {
                    ...
                    else
                    {
                        ...
                        cthdn.ThanhTien = decimal.Parse(txbThanhTien.Text);
        ...
        private void dataGridViewHDN_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //HienThiHoaDonNhapTextBox(vitri, HienThiHoaDonN());
            HienThiHoaDonNhapTextBox(vitri, bus.HienThiHDN(""));
            flag = 2;
        ...
        private void dataGridViewCTHDN2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                txbThanhTien.Text = row.Cells["ThanhTien1"].Value.ToString();
                txbThanhTien.Text = string.Format("{0:#,##0}", decimal.Parse(txbThanhTien.Text));
            }
            catch
            {
        ...
        private void btnInHoaDon_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridViewCTHDN2.Rows.Count - 0; i++)
            {
                HoaDonNhap hoaDonNhap = new HoaDonNhap
                {
                    ...
                    TongThanhToan = decimal.Parse(txbTongThanhToan.Text),
                    TenNCC = comboBoxNCC.Text,
                    TenNSX = comboBoxNSX.Text,
        ...
        {
            if (comboBoxNCC.SelectedIndex >= 0 && comboBoxNSX.SelectedIndex >= 0)
            {
                var maNCC = comboBoxNCC.SelectedValue.ToString();
                var maNSX = comboBoxNSX.SelectedValue.ToString();

                var dt = lk.GetByNCCAndNSX(maNCC, maNSX);
                comboBoxTenLK.DataSource = dt;
                comboBoxTenLK.DisplayMember = "TenLK";
                comboBoxTenLK.ValueMember = "MaLK";
            }
            else
            {
                comboBoxTenLK.DataSource = null;
            }
        }
