...
namespace DAL
{
    public class HoaDon_DAL
    {
        ...
        {
            // 1. Cộng lại số lượng tồn cho mỗi linh kiện của hóa đơn này
            string sqlUpdateStock = @"
        UPDATE lk
        SET lk.SoLuongTon = lk.SoLuongTon + d.SoLuong
        FROM LinhKien lk
        INNER JOIN CT_HoaDonBanHang d
            ON lk.MaLK = d.MaLK
        WHERE d.MaHDBH = N'" + ex.MaHDBH + @"'
          AND d.TrangThai = N'1'";

            KetNoi.ExecuteReader(sqlUpdateStock);

            // 2. Đánh dấu chi tiết hóa đơn là đã xoá (TrangThai = '0')
            string sqlDeleteDetail = @"
        UPDATE CT_HoaDonBanHang
        SET TrangThai = N'0'
        WHERE MaHDBH = N'" + ex.MaHDBH + @"'";

            KetNoi.ExecuteReader(sqlDeleteDetail);
        }
